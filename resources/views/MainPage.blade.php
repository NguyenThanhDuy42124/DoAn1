    ...
    <section class="container my-5 text-center">
        ...
        <div class="row g-4">
            ...
            <div class="col-6 col-md-3 shadow-sm">
             <a href="{{ route('products.list', ['category' => $cat->id])}}" class="text-decoration-none text-dark">
        ...
        <div class="row g-4">
            ...
            <div class="col-6 col-md-3">
                ...
                <div class="card product-card h-100 shadow-sm d-flex flex-column">
                    ...
                    {{-- === BỌC LINK BẮT ĐẦU === --}}
                    <a href="{{ route('products.detail', ['id' => $product->id]) }}" class="text-decoration-none text-dark" style="flex-grow: 1;">
                        ...
                        <img src="{{ $product->images->isNotEmpty()
                                    ...
                                    : asset('storage/product_images/default.jpg') }}" alt="{{ $product->name }}" class="card-img-top product-img" style="height: 200px; object-fit: cover;"> {{-- Đồng bộ style ảnh --}}
                    ...
                    <div class="card-footer bg-white border-top-0 pt-0 pb-3">
                        <div class="d-flex">
                            ...
                            <a href="{{ route('products.detail', ['id' => $product->id]) }}" class="btn btn-outline-primary btn-sm me-2 w-100" title="Xem chi tiết">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <a href="{{ route('general.users.edit', Auth::user()->id) }}" class="btn btn-warning btn-sm " title="Cập nhật thông tin">
                                ...
                                <form action="{{ route('buyer.carts.store') }}" method="POST">
                                    ...
                                    <button type="submit" class="btn btn-success btn-sm" title="Thêm vào giỏ">
                                ...
                                @endif
                                @else
                                <a href="{{ route('login') }}" class="btn btn-success btn-sm w-100" title="Thêm vào giỏ">
                                    <i class="fas fa-cart-plus"></i>
                                </a>
                                @endif
                            </div>
        ...
        <div class="row g-4">
            ...
            <div class="col-6 col-md-3 text-center">
                <div class="p-4 bg-light rounded shadow-sm">
                    <img src="{{ !empty($shop->img) ? asset('storage/' . $shop->img) : asset('storage/profile_images/default.jpg') }}" alt="{{ $shop->name }}" class="rounded-circle" style="width: 80px; height: 80px; object-fit: cover;">

                    <h6 class="mt-3">{{ $shop->name }}</h6>
                    <p class="small mb-2 {{ $shop->seller_reviews_avg_rating ? 'text-warning fw-bold' : 'text-muted' }}">
                        <i class="fas fa-star"></i>
                        {{ $shop->seller_reviews_avg_rating ? round($shop->seller_reviews_avg_rating, 1) : 'Chưa có' }}
                    </p>
                    <a href="{{ route('shop.show', ['id' => $shop->id]) }}" class="btn btn-sm btn-outline-primary">Xem cửa hàng</a>
                </div>
            </div>
            @empty
